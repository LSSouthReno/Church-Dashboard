...
function getCommunityGroupRowsForMonths_(months) {
  ...
  return months.map(m => {
    ...
    activeGroups.forEach(g => {
      ...
      mems.forEach(mem => {
        if (isActiveInRange_(mem.attributes, monthStart, monthEnd)) memberCount++;
      });
    });

    Logger.log('     ' + m.label + ': groups=' + activeGroups.length + ', members=' + memberCount);
...
function syncSundayPlansData_() {
  ...
  const serviceTypes = pcoTryGetAll_('/services/v2/service_types?per_page=100') || [];
  const worshipTypes = serviceTypes.filter(function(st) {
    const name = String((st.attributes && st.attributes.name) || '').toLowerCase();
    return name.includes('worship gathering') || name.includes('worship service');
  });

  if (!worshipTypes.length) {
    Logger.log('   syncSundayPlansData_: no worship gathering service types found');
    return;
  }

  Logger.log('   Sunday plan types: ' + worshipTypes.map(function(t){ return t.attributes.name; }).join(', '));

  const plansByDate = {}; // dateKey -> [serviceObj, ...]

  worshipTypes.forEach(function(st) {
    const typeId   = String(st.id);
    const typeName = String((st.attributes && st.attributes.name) || '').trim();
    ...
    plans.forEach(function(plan) {
      const sortDate = String((plan.attributes && plan.attributes.sort_date) || '');
      if (!sortDate) return;
      const datePart = sortDate.substring(0, 10);

      const planId   = String(plan.id);
      const planAttr = plan.attributes || {};

      // Team members (volunteers, captains, preacher)
      const members = pcoTryGetAll_(
        '/services/v2/service_types/' + typeId +
        '/plans/' + planId +
        '/team_members?per_page=100'
      ...
      members.forEach(function(tm) {
        ...
        var teamName = String(a.team_name || '').trim();
        var pos      = String(a.position_display_name || a.position || '').trim();
        ...
        if (pl.indexOf('preach') !== -1 || tl.indexOf('preach') !== -1) {
          preacher = name;
        } else if (pl.indexOf('captain') !== -1 || pl.indexOf('service lead') !== -1 || tl.indexOf('captain') !== -1) {
          captains.push(name);
        }

        if (status !== 'D') {
          volunteers.push({ name: name, team: teamName, pos: pos, status: status });
        }
      });

      // Order-of-service items — fetch only for the first (8am) service to save API calls
      var orderItems = [];
      var typeNameLow = typeName.toLowerCase();
      if (typeNameLow.indexOf('8am') !== -1 || typeNameLow.indexOf('8 am') !== -1 || worshipTypes[0] === st) {
        ...
        orderItems = items
          ...
          .map(function(i) {
            var a     = i.attributes || {};
            var lenMs = Number(a.length) || 0;
            return {
              seq:         Number(a.sequence) || 0,
              title:       String(a.title || '').trim(),
              type:        String(a.item_type || 'item'),
              lengthMin:   lenMs > 0 ? Math.round(lenMs / 60000) : 0,
  ...
  // Sort services within each date by service time (8am < 9:30am < 11am)
  function svcOrder(name) {
    var n = (name || '').toLowerCase();
    return n.indexOf('8am') !== -1 || n.indexOf('8 am') !== -1 ? 0
         : n.indexOf('9:30') !== -1 || n.indexOf('9am')  !== -1 ? 1
         : n.indexOf('11')   !== -1 ? 2 : 3;
  }

  var result = Object.keys(plansByDate).sort().map(function(date) {
    var d = new Date(date + 'T12:00:00');
    plansByDate[date].sort(function(a, b) { return svcOrder(a.serviceType) - svcOrder(b.serviceType); });
    return {
      date:     date,
      ds:       d.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
      services: plansByDate[date]
    };
  });

  // Write to SundayPlans sheet (one row per Sunday, JSON payload)
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sh = ensureSheet_(ss, 'SundayPlans', ['Date', 'JSON']);
  sh.clearContents();
  sh.getRange(1, 1, 1, 2).setValues([['Date', 'JSON']]);
  if (result.length) {
    var rows = result.map(function(p) { return [p.date, JSON.stringify(p)]; });
    sh.getRange(2, 1, rows.length, 2).setValues(rows);
  }
  sh.setFrozenRows(1);

  Logger.log('   SundayPlans: ' + result.length + ' upcoming Sundays cached');
}
...
function getSundayPlans_() {
  ...
  return rows.map(function(r) {
    try { return JSON.parse(String(r[1])); } catch(e) { return null; }
  }).filter(Boolean);
}
